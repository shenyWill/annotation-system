$(function(){
    var invoiceAnnotation = {
        ...
        //             _self.selectArr = [
        //                 {key:'border', value: '外边框'},
        //                 {key:'office', value: '签发机关'},
        //                 {key:'date', value: '有效日期'}
        //                ]
        //         }else {
        //             _self.selectArr = [
        ...
        invoiceTypeAjax: function(){
        	var _self = this;
            $.ajax({
                url: '/subscriber/globalset/getLabelType',
                contentType: 'application/json',
                data: JSON.stringify({}),
                type: 'post',
                success: function(data){
                    if(data.labelType == 3) {
                    	_self.selectArr =  [{key:'invoiceCode', value: '发票代码'},{key:'invoiceNum', value: '发票号码'},{key:'invoiceDate', value: '开票日期'},{key:'totalMoney', value: '合计金额'},{key:'totalInvoiceMoney', value: '合计税额'},{key:'allMoney', value: '价税合计'},{key:'checkCode', value: '校验码'}];
                    }
                    if(data.labelType == 2) {
                    	_self.selectArr =  [{key:'invoiceCode', value: '发票代码'},{key:'invoiceNum', value: '发票号码'},{key:'invoiceDate', value: '开票日期'},{key:'totalMoney', value: '合计金额'},{key:'totalInvoiceMoney', value: '合计税额'},{key:'allMoney', value: '价税合计'}];
                    }
                    if(data.labelType == 4) {
                    	_self.selectArr =  [{key:'openDate', value: '开票日期'},{key:'invoiceCode', value: '发票代码'},{key:'invoiceNum', value: '发票号码'},{key:'carSmallMoney', value: '车价合计小写'},{key:'carBigMoney', value: '车价合计大写'}];
                    }
                    if(data.labelType == 1) {
                    	_self.selectArr =  [
                            {key:'idNum', value: '身份证号码'},
                            {key:'border', value: '外边框'},
                            {key:'username', value: '姓名'},
                            {key:'sex-race', value: '性别和民族'},
                            {key:'birthday', value: '出生日期'},{key:'addressFirst', value: '住址第一行'},
        ...
        initImage: function() {
            var _self = this;
            var imageType = $('.annotate-image').attr('type');
            $.ajax({
                url: '/subscriber/imagelabelInfoInvoice/imagelabel',
                contentType: 'application/json',
                data: JSON.stringify({type: imageType}),
                type: 'post',
                success: function(data){
                    var img = new Image();
                    img.src = data.imageUrl;
                    $('.annotate-image').attr('id', data.id); // fix image id bugs, hacks
                    img.onload = function(){
        ...
        drawMoveFrame: function(){
            ...
            $('.upper-canvas').on('mousedown', function(e){
                ...
                $(document).bind('mouseup', function (event) {
                    ...
                    _self.pointArray[1] = {x: event.offsetX, y: e.offsetY};
                    _self.pointArray[2] = {x: event.offsetX, y: event.offsetY};
                    _self.pointArray[3] = {x: e.offsetX, y: event.offsetY};
        ...
        drawContent: function() {
            ...
            $('.content-select').each(function(){
                if($(this).val() == 'idNum') {
                    _self.IDNum = $(this).siblings('.content-text').val();
                }
            })
        ...
        upload: function() {
            ...
            $('.upload-btn').on('click', function(){
                ...
                var imageType = $('.annotate-image').attr('type');
                var invoiceDate = {id: id, temp1:_self.invoiceArray, type: imageType};
                // 判断是否有重复
